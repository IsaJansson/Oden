...
* Calculates the base_url and stores all the useful details in $this.
* @package OdenCore
*/
...
class CRequest {
	...
	/*
	* Constructor 
	* Deciedes which type of url should be generated as outgoing links.
    ...
    */
	...
	/*
	* Initioalizing the object by parsing the current url request.
	*/
	public function Init($base_url = null) {
		...
		$controller = !empty($spilts[0]) ? $spilts[0] : 'index';
		$method = !empty($splits[1]) ? $spilts[1] : 'index';
		$arguments = $splits;
		unset($arguments[0], $arguments[1]); // Remove the method and controller part of the arguments list
